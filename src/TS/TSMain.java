...
public class TSMain {
	...
	public void init() {
		...
		year = 0;
		File file = new File(
				"C:\\Users\\watwat\\Desktop\\2015\\2014\\CS研究\\2ndday\\"+name);
		 ...
		 try{
			 Calendar now = Calendar.getInstance();
		      File file = new File(SaveName+"\\"
						+ "Result" + +now.get(Calendar.DAY_OF_MONTH)+now.get(Calendar.HOUR_OF_DAY)
			...
			for(int jobIndexX = 0; jobIndexX < data.getJobCount()-1; jobIndexX++){
				for(int jobIndexY = jobIndexX +1; jobIndexY < data.getJobCount(); jobIndexY++){
					if(!machines.isTabu(machineIndex, jobIndexX, jobIndexY, year)){
						...
						//System.out.println(machineIndex + " :" + jobIndexX + ":" + jobIndexY);
	...
	public void outResult(){
		...
		try {
			...
			oos = new ObjectOutputStream(new FileOutputStream(SaveName+"\\"
					+ "Result" + +now.get(Calendar.DAY_OF_MONTH)+now.get(Calendar.HOUR_OF_DAY)
					+ now.get(Calendar.MINUTE) + ".ts"));
